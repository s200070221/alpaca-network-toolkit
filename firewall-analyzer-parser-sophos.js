...
const SophosParser = (() => {
  ...
  function parsePolicies(xml,addrTypeMap){
    ...
    ['FirewallRule','SecurityPolicy'].forEach(tag=>xblks(xml,tag).forEach((b,idx)=>{
      ...
      policies.push({id:xv(inner,'Id')||String(idx+1),name,srcIntf:xv(inner,'SourceZone')||xv(inner,'FromZone')||'-',dstIntf:xv(inner,'DestinationZone')||xv(inner,'ToZone')||'-',srcAddr:srcAddrStr,dstAddr:dstAddrStr,srcAddr4:srcAddrSplit.v4,srcAddr6:srcAddrSplit.v6,dstAddr4:dstAddrSplit.v4,dstAddr6:dstAddrSplit.v6,service:svcs.join(', ')||'Any',schedule:xv(inner,'Schedule')||'All the time',action:action.includes('accept')||action.includes('allow')||action.includes('permit')?'accept':'deny',nat:xv(inner,'OutboundNAT')||(xv(inner,'NATRule')?'enable':'disable'),ippool:'disable',poolname:xv(inner,'NATRule')||'-',logtraffic:xv(inner,'LogTraffic')||xv(inner,'Log')||'disable',logstart:'-',utm:{av:xv(inner,'AntiVirus')||xv(inner,'AVProfile')||'-',webfilter:xv(inner,'WebFilter')||xv(inner,'WebPolicy')||'-',ips:xv(inner,'IPS')||xv(inner,'IPSPolicy')||'-',ssl:xv(inner,'SSLInspection')||'-',appctrl:xv(inner,'ApplicationControl')||xv(inner,'AppPolicy')||'-'},status:(xv(inner,'Status')||xv(inner,'Active')||'Enable')==='Disable'?'disable':'enable',comments:xv(inner,'Description')||xv(inner,'Comments')||'-',users:plist(inner,'Users').join(', ')||'-',groups:plist(inner,'Groups').join(', ')||'-'});
    }));
    return policies;
  }
  ...
  function parseSdwan(xml) {
    ...
    wanLinks.forEach((lk, idx) => {
      ...
      result.members.push({
        ...
        gateway6: '-',
        priority: (v=>v!==''?parseInt(v):1)(xv(t,'Priority')||''),
        weight:   (v=>v!==''?parseInt(v):1)(xv(t,'Weight')||''),
