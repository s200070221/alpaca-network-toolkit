function parseProCurve(cfg) {
  ...
  function parseVlans() {
    ...
    for (const b of cfg.split(/^(?=vlan\s+\d)/m)) {
      ...
      // 次要IP（2026-08-12 新增，2026-08-17 從「僅取第一筆」擴大為完整收集，中信心度：
      // 多個獨立來源交叉確認 ProCurve 無 secondary 關鍵字，同一 VLAN 上第一個手動設定的
      // IP 是主要位址，後續設定的（不同子網）自動變成次要位址，最多 7 個；官方 HP
      // "Configuring IP Addressing" 章節確認機制存在，惟 WebFetch 對該 PDF 解析不完整
      // 未能取得逐字指令範例）
      ...
      // secondaryIps 比照上面 ip 欄位既有慣例（原樣 dotted-mask 字串以 / 相接，非真正 CIDR
      // prefix），與 renderProCurveVLANs() 的 v.secondaryIps 還原邏輯一致，最多保留官方
      // 確認的 7 個上限
      const secondaryIps=ipMatches.slice(1,7).map(m=>`${m[1]}/${m[2]}`);
      vlans.push({ id, name, tagged, untagged, noUntagged, ports:[], ip:mIp?`${mIp[1]}/${mIp[2]}`:'', secondaryIps, ipSubnets });
    }
    return vlans;
  }
