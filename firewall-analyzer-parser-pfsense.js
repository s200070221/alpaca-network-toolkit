...
const PfsenseParser = (() => {
  ...
  // 次要IP（IP Alias VIP，2026-08-12 新增，2026-08-17 從「僅取第一筆」擴大為完整收集，
  // 官方 docs.netgate.com "Virtual IP Addresses" 頁面確認 <mode>ipalias</mode> 為次要IP
  // 機制；VIP 是獨立於 <interfaces> 之外的頂層 <virtualip><vip>...</vip></virtualip> 區塊，
  // <interface> 值是內部代稱（wan/lan/optN，與 buildIfMap() 的 key 同一套命名空間），非
  // 真實介面名稱 realif，故依內部代稱分組合併回對應介面）
  function parseSecondaryIps(xml) {
    ...
    ipaliasVips.forEach(b => {
      ...
      if (subnet) (result[ifKey] = result[ifKey] || []).push({ ip: subnet, mask: bits ? prefixToMask(parseInt(bits)) : '255.255.255.255' });
    });
    return result;
  }
  ...
  function parseInterfaces(xml) {
    ...
    Object.entries(ifMap).forEach(([key, ifc]) => {
      ...
      ifaces.push({
        ...
        ip:      ip !== '-' ? ip : '-',
        mask,
        secondaryIps: secondaryIpMap[key] || [],
